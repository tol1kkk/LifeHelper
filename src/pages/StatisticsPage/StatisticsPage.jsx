...
export default function StatisticsPage({ tasks = [] }) {
  ...
  const taskChartData = [
    { label: "Mon", value: Math.max(1, Math.min(totalTasks, 12)) },
    { label: "Tue", value: Math.max(2, Math.min(completedTasks + 3, 12)) },
    { label: "Wed", value: Math.max(1, Math.min(totalTasks + 2, 12)) },
    { label: "Thu", value: Math.max(3, Math.min(completedTasks + 5, 12)) },
    {
      label: "Fri",
      value: Math.max(4, Math.min(totalTasks + completedTasks, 12)),
    },
    { label: "Sat", value: Math.max(2, Math.min(completedTasks + 2, 12)) },
    { label: "Sun", value: Math.max(1, Math.min(totalTasks + 1, 12)) },
  ];
  ...
  const maxChartValue = Math.max(...taskChartData.map((item) => item.value), 1);

  const chartPoints = taskChartData.map((item, index) => {
    const x =
      chartPadding +
      (index * (chartWidth - chartPadding * 2)) / (taskChartData.length - 1);
      ...
      <section className="chartCard">
        <div className="statisticsSectionHeader">
          <div>
            ...
            <p>Task activity for this {selectedPeriod.toLowerCase()}</p>
          </div>
